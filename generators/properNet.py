...
def test():
    #open a text file
    path = "../datasets/singleFinger.csv"
    ...
    rnn = RNN(len(chars), len(chars), 20, 0.1)
    ...
    epochs = 200
    for iterNumb in range(int(len(data)/seq_length) * epochs):
        i = iterNumb % int(len(data)/seq_length)
        ...
        if iterNumb%100==0:
            print( '---\niteration %d, smooth_loss = %f \n---' % (iterNumb, smooth_loss))
            losses.append(smooth_loss)
            with open('rnn.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
                pickle.dump(rnn, f)
